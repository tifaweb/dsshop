...
//	});
	...
	$('#file_upone').uploadify({
        ...
        'onUploadSuccess' : function(file, data, response) {//每次成功上传后执行的回调函数，从服务端返回数据到前端
               img_id_upload[i]=data;
               i++;
				$("#file_up_content").html("<li id='i"+i+"'><input name='i_img' type='hidden'  value='"+data+"'/><a id='feila"+i+"' ><img id='feil"+i+"' src='"+fl+data+"' style='width:150px;height:150px;'></li>");
        },
        'onQueueComplete' : function(queueData) {//上传队列全部完成后执行的回调函数
            //if(img_id_upload.length>0)
            //alert('成功上传的文件有：'+encodeURIComponent(img_id_upload));
	...
	var file_url=$("#file_url").val()?$("#file_url").val():' ';
	var file_delete=$("#file_delete").val()?$("#file_delete").val():' ';	//如果存在图片就删除
	$('#file_upload').uploadify({
		'formData'     : {
			'folder' : folder,
			'file_delete' : ''+file_delete+'',
	...
	var file_urls=$("#file_urls").val()?$("#file_urls").val():' ';
	$('#file_good').uploadify({
		'formData'     : {
			'folder' : folder,
			'timestamp' : timestamp,
